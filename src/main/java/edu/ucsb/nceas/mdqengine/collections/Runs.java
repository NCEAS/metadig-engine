...
public class Runs {
    ...
    /**
     * Determine the correct sequenceId for this run by finding the sequenceId
     * assigned to previous pids
     * in this obsolescence chain.
     * <p>
     * Evaluate the sequenceId of all runs in the DataONE obsolescence chain and
     * determine the correct
     * seriedId to use for the current pid. If one doesn't exist, then create a new
     * one.
     * </p>
     *
     * @throws Exception
     ...
     * <p>
     * Recursion ends when either the next run is not available (not in store) or
     * the pointer to the
     * next pid in the chain isn't specified (we are at the end of the chain).
     * </p>
     * 
     * @param metadataId      The identifier of the metadata document associated
     *                        with the report
     * @param suiteId         The identifier for the suite used to score the
     *                        metadata
    ...
    public void getNextRun(String metadataId, String suiteId, Boolean stopWhenSIfound, MDQStore store, Boolean forward,
        ...
        if (run == null) {
            try {
                ...
                // the 'store.getRun' returns null if the run isn't found in the store. This
        ...
        // If a run was found for this pid in the chain, check if a sequence id was
        // previously
        // defined for it. We want to use the same sequence id for all pids in the
        // chain, right?
        if (run != null) {
            ...
            if (sequenceId != null) {
                ...
                if (this.sequenceId != null) {
                    if (!this.sequenceId.equals(sequenceId)) {
                        log.error("Warning, new sequenceId found for chain: " + sequenceId + " found at pid: "
                                + metadataId);
                    }
                } else {
            ...
            // Get the sysmeta object within the run, to retrieve the 'obsoletes' or
            // 'obsoletedBy' pid
            ...
            if (forward) {
                ...
                if (obsoletes != null) {
                    ...
                    if (obsoletes.compareToIgnoreCase(metadataId) == 0) {
                        log.debug("Stopping traversal at invalid obsoletes, pid " + metadataId
                                + " is obsoleted by itself");
                        return;
                    }
            ...
            log.debug("Run not found in store for pid: " + metadataId + ", suiteId: " + suiteId
                    + ", terminating search in current direction.");
        }
     ...
     * <p>
     * Successive versions of a metadata document are represented in DataONE as pids
     * comprising an obsolecence chain (i.e. linked list) or sequence,
     * where new versions 'obsolete' older, outdated ones.
     * This method follows the obsolecence chain to retrieve all runs corresponding
     * to pids in the obsolescence chain.
     * The least number of pids to get will be for all versions that are within the
     * current month of the starting pid.
     * If <b>stopWhenSIfound</b> is specified, the search will continue until the
     * sequenceId is found, or the entire chain is
     * fetched.
     *
     * A new sequenceId can only be assinged to the first version in a chain. If
     * pids are processed out of 'dateUploaded' order,
     * i.e. due to multi-processing, there may be 'breaks' in the chain and pids may
     * temporarily be without sequenceIds. When a
     * new pid is added, a check will be made to re-link pids back to the chain by
     * traversing backward until a sequenceId is
     * found, then previously saved pids with no sequenceId will have the correct
     * one assinged.
     * </p>
     *
    ...
    public void getRunSequence(Run run, String suiteId, Boolean stopWhenSIfound) throws MetadigException {
        ...
        try (DatabaseStore store = new DatabaseStore()) {
            ...
            // If the sequenceId has not been obtained when searching in the backward
            // direction, then there
            // is no reason to search forward, as this means that the first pid in the
            // series has not been found,
            ...
            log.debug("Done getting all runs (in DataONE obsolescence chain) for : metadata PID: " + metadataId
                    + ", suite id: " + suiteId);
        }
    }
     ...
     * <p>
     * The runs in a sequence have already been fetched via <b>getRunsInSeq</b>, but
     * some of them may not
     * have been assigned a sequenceId, i.e. possibly due to broken chains that have
     * now been joined. Check
     * each pid in the sequence and assign it the sequenceId if it doesn't have it
     * already.
     * </p>
     *
     * @param sequenceId a quality engine maintained sequence identifier, similiar
     *                   in function to the DataONE series id.
     */
    ...
    /**
     * <p>
     * Determine which run in this obsolecense sequence is the latest in the month,
     * give a date.
     * </p>
     *
    ...
    public void setLatestRunInMonth(Date date) {
        ...
        for (Map.Entry<String, Run> entry : this.runs.entrySet()) {
            ...
            log.debug("Checking run with pid: " + thisPid + ", dateUploaded: " + dateUploaded + ", isLatest: "
                    + latestSet);
            ...
            if (thisPid.equalsIgnoreCase(latestPid)) {
                log.info("Setting latest run in month to pid: " + run.getObjectIdentifier() + " with date: "
                        + run.getDateUploaded());
                ...
                if (latestSet) {
                    log.info("Unsetting latest run in month to pid: " + run.getObjectIdentifier() + " with date: "
                            + run.getDateUploaded());
    ...
    /**
     * <p>
     * Return the run from the collection that is the latest in the given month. The
     * field 'dateUploaded' is used for comparision. Since all runs in the
     * collection are
     * in the same DateONE obsolecense chain, returning the 'latest' in a given
     * month
     * should correspond to the pid with the highest metadata quality score.
     * Finding this run (pid) can be useful to aggregation and display routines to
     * filter out
        ...
        for (Map.Entry<String, Run> entry : this.runs.entrySet()) {
            ...
            if (modified) {
                try {
                    ...
                    run.save();
                    // Keep modified setting for modified runs in case we need to do other
                    // operations on the runs (e.g. indexing)
                    // this.runs.replace(thisPid, run);
                } catch (Exception ex) {
    ...
    /**
     * If pids are missing in the current obsolesence chain, then a traversal may
     * not reach the starting pid, i.e.
     * the very first one in the chain. This must be determined, as we only assign a
     * sequenceId when the first pids is found.
     *
     * @return foundFirstPid - was the starting pid in this obsolesense chain
     *         reached?
     */
