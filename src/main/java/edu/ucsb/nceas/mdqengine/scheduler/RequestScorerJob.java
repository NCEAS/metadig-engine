...
public class RequestScorerJob implements Job {
     ...
     * </p>
     * <p>
     * This method sends a request from the scheduler (metadig-scheduler) to the
     * controller (metadig-controller)
     * to execute a 'scorer' request (metadig-scorer). This request goes through the
     * controller so that it can
     * keep track and log all requests and their completion status. The current
     * method to send requests to the
     * controller is to send a REST request to the servlet running the controller,
     * using the metadig-engine API.
     * </p>
     *
    ...
    public void execute(JobExecutionContext context)
        ...
        // Number of pids to get each query (this number of pids will be fetched each
        // query until all pids are obtained)
        ...
        try {
            ...
            JobExecutionException jee = new JobExecutionException(
                    taskName + ": Error executing task: " + ce.getMessage());
            jee.initCause(ce);
            throw jee;
        }
        ...
        try (DatabaseStore store = new DatabaseStore()) {
            // TODO: consider removing this block not sure if it's ever hit
            ...
            if (requestType != null && requestType.equalsIgnoreCase("node")) {
                try {
                    ...
                    // the nodeId as the collectionid. It is not necessary to retrieve a
                    // collectionQuery for this
                    // 'node' portal, as there is no Solr entry for this type collection. All
                    // quality scores available
                    // in the quality Solr server will be directly retrieved, filtering on the
                    // 'nodeId' (datasource)
                    log.info("TaskName: " + taskName + ", taskType: " + taskType
                            + " submitting node request for nodeId: "
                            + nodeId + ", suiteId: " + suiteId + "formatFamily: " + formatFamily);
                    submitScorerRequest(qualityServiceUrl, nodeId, suiteId, nodeId, formatFamily);
                } catch (Exception e) {
                    JobExecutionException jee = new JobExecutionException(
                            "Unable to submit request to create new node ("
                                    + nodeId + ")" + " score graph/data file ",
                            e);
                ...
                while (morePids) {
                    // Get a list of pids selected by a collection (portal) search filter
                    // (collectionQuery) and get
                    // the quality scores (from the quality Solr server) for that list of pids.
                    ArrayList<String> pidsToProcess = null;
                    log.trace("Getting portal pids to process, startCount: " + startCount + ", countRequested: "
                            + countRequested);

                    try {
                        result = getPidsToProcess(d1Node, session, pidFilter, startDTstr, endDTstr, startCount,
                                countRequested, lastDateModifiedDT, taskName);
                    ...
                    log.trace(taskName + ": found " + resultCount + " seriesIds" + " for date: " + startDTstr
                            + " at servierUrl: " + nodeServiceUrl);
                    for (String pidStr : pidsToProcess) {
                        try {
                            ...
                            JobExecutionException jee = new JobExecutionException(
                                    "Unable to submit request to create new score graph/data file", e);
                            jee.setRefireImmediately(false);
                            throw jee;
                        }
                    }
                ...
                if (allIds > 0) {
                    ...
                    log.info(taskName + ": found " + allIds + " seriesIds" + " for start: " + startDTstr + ", end: "
                            + endDTstr + " at servierUrl: " + nodeServiceUrl);
                }
            }
        } catch (Exception e) {
    ...
    /**
     * Query a DataONE CN or MN object store for a list of object that match the
     * time range and formatId filters provided.
     *
    ...
    /**
     * Submit a requst to the metadig controller to get quality score info and
     * create a graph for the specified collection.
     *
     ...
     * @param nodeId            the DataONE node identifier that the collection is
     *                          hosted on
     * @param formatFamily      the format identifier family (e.g. "eml" for all EML
     *                          format identifier versions)
     *
     * @throws Exception
     *
     */
    public void submitScorerRequest(String qualityServiceUrl, String collectionId, String suiteId, String nodeId,
            String formatFamily) throws Exception {
