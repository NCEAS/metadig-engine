            ...
            */
            ...
            for (Node node : nodes) {
                ...
                if (task.getLastHarvestDatetime(harvestNodeId) == null) {
                    ...
                    // save the task as a checkpoint here to make sure we always keep track of tasks
                    ...
                    try {
                        ...
                        if (isCN) {
                            ...
                            objList = cnNode.listObjects(session, startDate, endDate, searchFormat, nodeRef, identifier, startCount,
                                ...
                                log.error(taskName + ": error retrieving pids for format " + searchFormat + ": " + e.getMessage());
                                throw e;
                            }
                                ...
                                for (ObjectInfo oi : objList.getObjectInfoList()) {
                                    ...
                                    try (DatabaseStore store = new DatabaseStore()) {
                                        ...
                                        if (!exists) {
                                            pidCount++; 
                                            pids.add(thisPid);
                                            log.trace("adding pid " + thisPid + ", formatId: " + formatId);
                                            
                                            // If this pid's modified date is after the stored latest encountered modified
                                            // date, then update the lastModified date
                                            ...
                                            // again (in the event that this is the last pid to be harvested in this round.
