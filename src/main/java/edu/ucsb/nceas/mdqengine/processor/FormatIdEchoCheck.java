...
/**
 * Extract formatId from given SystemMetadata
 * @author leinfelder
 *
 */
public class FormatIdEchoCheck implements Callable<Result> {
	...
	public Result call() {
		...
		if (this.systemMetadata != null) {
			...
			try {
				...
				// get the formatId
				result.setOutput(new Output(sm.getFormatId().getValue()));
